...
SUDO_USERS = [7678359785]  # Sudo users who can bypass bans
...
async def check_ban(client: Client, message: t.Message):
    ...
    if is_banned(user_id):
        ...
        if end_time:
            ...
            await message.reply_text(f"You are banned from using commands until {ban_end}. Appeal at @lrithyll", quote=True)
        else:
            await message.reply_text("You are permanently banned from using commands. Appeal at @lrithyll", quote=True)
